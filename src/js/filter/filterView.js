...
export function render(params){
    ...
    params.complexNames.forEach(item =>{
        complexNames += `<option value=${item}>${item}</option> `
    })
    let rooms = ''
...
export function changeButtonFilter(number){
    elements.filterSubmit[0].innerText = `Показать ${number} объектов`

    elements.filterSubmit[0].disabled = number === 0 ? true:false
}
